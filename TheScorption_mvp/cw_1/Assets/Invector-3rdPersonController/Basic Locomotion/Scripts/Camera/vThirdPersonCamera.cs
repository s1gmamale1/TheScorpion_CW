...
namespace Invector.vCamera
{
    ...
    public class vThirdPersonCamera : MonoBehaviour
    {
        ...
        [SerializeField] protected float _smoothCameraRotation = 20f;
        ...
        public virtual void RotateCamera(float x, float y)
        {
            ...
            if (!currentState.cameraMode.Equals(TPCameraMode.FixedAngle))
            {
                ...
                if (!lockTarget)
                {
                    ...
                    {
                        if (autoBehindTarget)
                            smoothCameraRotation = Mathf.Lerp(smoothCameraRotation, behindTargetSmoothRotation, 6f * Time.deltaTime);
        ...
        public virtual void CameraMovement(bool forceUpdate = false)
        {
            ...
            transformWeight = Mathf.Clamp(transformWeight += Time.deltaTime * startSmoothFactor, 0f, 1f);
            if (useSmooth)
            {
            ...
            currentSwitchRight = Mathf.Lerp(currentSwitchRight, switchRight, smoothSwitchSide * Time.deltaTime);
